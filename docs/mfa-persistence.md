...
The service persists enrollments, challenges, and the MFA assurance outbox in PostgreSQL. Flyway migration `V1__create_mfa_persistence.sql` creates the `mfa_enrollments` and `mfa_challenges` tables, `V2__add_transfer_challenge_binding.sql` adds the immutable transfer decision binding, and `V3__create_mfa_assurance_outbox.sql` adds the durable assurance event table.
...
The outbox stores the complete serialized `MfaAssuranceGranted.v1` payload and event id before the transaction commits. The Kafka publisher marks a row published only after broker acknowledgement. A failed publish leaves the row pending, so a retry sends the same topic key and exact payload; duplicate delivery is expected and consumers deduplicate by event id.
